...
export function MessageList(_a) {
    var messages = _a.messages, isLoading = _a.isLoading, error = _a.error;
    if (isLoading) {
    ...
    return (_jsx(ScrollView, { flex: 1, contentContainerStyle: { paddingBottom: 20 }, children: _jsx(YStack, { gap: "$2", p: "$3", children: messages.map(function (message) { return (_jsx(MessageItem, { message: message }, message.ts)); }) }) }));
}
function MessageItem(_a) {
    var message = _a.message;
    // Parse timestamp from Slack (in seconds) to Date
    var timestamp = new Date(Number(message.ts) * 1000);
    var timeText = formatDistanceToNow(timestamp, { addSuffix: true });
    var user = message.user;
    var userName = (user === null || user === void 0 ? void 0 : user.real_name) || (user === null || user === void 0 ? void 0 : user.display_name) || (user === null || user === void 0 ? void 0 : user.name) || 'Unknown user';
    var avatarUrl = (user === null || user === void 0 ? void 0 : user.image_192) || '';

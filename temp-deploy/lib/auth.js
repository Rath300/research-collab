var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
import { getSupabaseClient } from './supabaseClient';
export function getCurrentUser() {
    return __awaiter(this, void 0, void 0, function () {
        var supabase, _a, user, authError, _b, profile, profileError, error_1;
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0:
                    supabase = getSupabaseClient();
                    _c.label = 1;
                case 1:
                    _c.trys.push([1, 4, , 5]);
                    return [4 /*yield*/, supabase.auth.getUser()];
                case 2:
                    _a = _c.sent(), user = _a.data.user, authError = _a.error;
                    if (authError || !user) {
                        return [2 /*return*/, null];
                    }
                    return [4 /*yield*/, supabase
                            .from('profiles')
                            .select('*')
                            .eq('id', user.id)
                            .single()];
                case 3:
                    _b = _c.sent(), profile = _b.data, profileError = _b.error;
                    if (profileError) {
                        console.error('Error fetching profile:', profileError);
                    }
                    return [2 /*return*/, {
                            id: user.id,
                            email: user.email,
                            profile: profile || null,
                        }];
                case 4:
                    error_1 = _c.sent();
                    console.error('Error getting current user:', error_1);
                    return [2 /*return*/, null];
                case 5: return [2 /*return*/];
            }
        });
    });
}
export function signIn(email, password) {
    return __awaiter(this, void 0, void 0, function () {
        var supabase, _a, data, error, error_2;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    supabase = getSupabaseClient();
                    _b.label = 1;
                case 1:
                    _b.trys.push([1, 3, , 4]);
                    return [4 /*yield*/, supabase.auth.signInWithPassword({
                            email: email,
                            password: password,
                        })];
                case 2:
                    _a = _b.sent(), data = _a.data, error = _a.error;
                    if (error) {
                        throw new Error(error.message);
                    }
                    return [2 /*return*/, data];
                case 3:
                    error_2 = _b.sent();
                    console.error('Error signing in:', error_2);
                    throw error_2;
                case 4: return [2 /*return*/];
            }
        });
    });
}
export function signUp(email, password, userData) {
    return __awaiter(this, void 0, void 0, function () {
        var supabase, _a, authData, authError, profileError, error_3;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    supabase = getSupabaseClient();
                    _b.label = 1;
                case 1:
                    _b.trys.push([1, 4, , 5]);
                    return [4 /*yield*/, supabase.auth.signUp({
                            email: email,
                            password: password,
                        })];
                case 2:
                    _a = _b.sent(), authData = _a.data, authError = _a.error;
                    if (authError || !authData.user) {
                        throw new Error((authError === null || authError === void 0 ? void 0 : authError.message) || 'Error creating user');
                    }
                    return [4 /*yield*/, supabase
                            .from('profiles')
                            .insert(__assign({ id: authData.user.id, first_name: userData.first_name || '', last_name: userData.last_name || '', email: email, joining_date: new Date().toISOString(), created_at: new Date().toISOString(), updated_at: new Date().toISOString() }, userData))];
                case 3:
                    profileError = (_b.sent()).error;
                    if (profileError) {
                        console.error('Error creating profile:', profileError);
                        throw new Error(profileError.message);
                    }
                    return [2 /*return*/, authData];
                case 4:
                    error_3 = _b.sent();
                    console.error('Error signing up:', error_3);
                    throw error_3;
                case 5: return [2 /*return*/];
            }
        });
    });
}
export function signOut() {
    return __awaiter(this, void 0, void 0, function () {
        var supabase, error, error_4;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    supabase = getSupabaseClient();
                    _a.label = 1;
                case 1:
                    _a.trys.push([1, 3, , 4]);
                    return [4 /*yield*/, supabase.auth.signOut()];
                case 2:
                    error = (_a.sent()).error;
                    if (error) {
                        throw new Error(error.message);
                    }
                    return [2 /*return*/, true];
                case 3:
                    error_4 = _a.sent();
                    console.error('Error signing out:', error_4);
                    throw error_4;
                case 4: return [2 /*return*/];
            }
        });
    });
}

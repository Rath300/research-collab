...
import { ResearchPost } from '@/lib/schema';
import { 
  FiUser, 
  FiThumbsUp, 
  FiMessageSquare, 
...
export function ResearchPostCard({ post, onLike, onBoost, onClick }: ResearchPostCardProps) {
  const {
    id,
    title,
    content,
  ...
  return (
    <Card className="mb-4 overflow-hidden hover:shadow-md transition-shadow duration-200">
      <CardContent className="p-4 space-y-4">
        ...
        {/* Post content */}
        <div>
          <Link href={`/research/${id}`} onClick={onClick}>

...
function ResearchPageContent() {
  ...
  const { posts: researchPosts, isLoading, hasMore, setPosts, appendPosts, setLoading, setHasMore, resetPosts } = useResearchStore();
  ...
  const loadPosts = async (reset = false) => {
    try {
      ...
      // Get posts from API
      const newPosts = await posts.getResearchPosts({
  ...
  const handleLike = async (postId: string) => {
    ...
    try {
      ...
      // Update in state
      setPosts(researchPosts.map((p) => (p.id === postId ? updatedPost : p)));
      
      // Update in the database
      await posts.updateResearchPost(postId, {
  ...
  return (
    <div className="container mx-auto max-w-4xl px-4 py-8">
      ...
      <div className="space-y-6">
        ...
        {!isLoading && !hasMore && researchPosts.length > 0 && (

      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden dark:bg-slate-800 p-8">
        ...
        <div className="mb-6">
          ...
          {apiKeys.length > 0 ? (
            <ul className="list-disc pl-6 space-y-1">
              {apiKeys.map((key) => (
                <li key={key}>
                  {key} - {typeof (api as any)[key]}
                </li>
              ))}
            </ul>
          ) : (
            <p className="text-gray-500">No keys found in API module</p>
          )}
        </div>
        ...
        <div>
          ...
          <ul className="list-disc pl-6 space-y-1">
            ...
            <li>
              <strong>Next.js:</strong> {(typeof window !== 'undefined' && (window as any).__NEXT_DATA__?.buildId) ? 'Running' : 'Unknown'}
            </li>
            <li>

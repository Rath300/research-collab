...
import { Profile, ResearchPost } from '@/lib/schema';
...
export default function ProfilePage() {
  ...
  const [userPosts, setPosts] = useState<ResearchPost[]>([]);
  ...
  const loadProfileData = async () => {
    try {
      ...
      // Get profile data
      const profileData = await profiles.getById(userId);
      setProfile(profileData);
      
      // Get user's research posts
      const userPosts = await posts.getByUserId(userId);
      setPosts(userPosts);
      
      // Check if already matched (if not own profile)
      if (user && !isOwnProfile) {
        const matchStatus = await matches.checkIfMatched(user.id, userId);
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      await matches.createMatch(user.id, userId);
      setIsMatched(true);
    } catch (err) {

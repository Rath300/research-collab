...
function ResetPasswordContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Store redirectTo in localStorage if it exists
      if (redirectTo) {
  ...
  // Create login URL with redirectTo if needed
  let loginUrl = redirectTo ? `/login?redirectTo=${encodeURIComponent(redirectTo)}` : '/login';
  
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-slate-900 px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <Link href={getSafeRedirectUrl('/')} className="inline-block">
        ...
        <Card>
          ...
          <div className="pb-6 px-6">
            <Link 
              href={getSafeRedirectUrl(loginUrl)}

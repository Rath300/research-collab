    ...
    try {
      ...
      if (!result.success) {
        ...
        Object.keys(formattedErrors).forEach(key => {
          if (key !== '_errors') {
            const errorField = key as keyof ProfileFormData;
            const errorMessages = formattedErrors[errorField]?._errors;
            if (errorMessages && errorMessages.length > 0) {
              newErrors[errorField] = errorMessages[0];
            }
          }
        });
          ...
          <div className="flex justify-between mt-8">
            ...
            {currentStep < totalSteps ? (
              <button
                ...
                className="px-6 py-2 bg-researchbee-yellow text-black rounded-md hover:bg-yellow-500 transition-colors ml-auto"
                disabled={isSubmitting}
              >
                Next
              </button>
